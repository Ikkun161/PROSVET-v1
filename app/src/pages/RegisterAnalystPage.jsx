...
import { Link, useNavigate } from 'react-router-dom'; // добавим useNavigate для редиректа
import regLogo from '../img/logo-reglog.svg';
import errorIcon from '../img/error-pass.svg';
import './AuthPages.css';

function RegisterAnalystPage() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [passwordError, setPasswordError] = useState(false);
  const [error, setError] = useState('');           // для ошибок с сервера
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Успех – сразу на заполнение профиля
  ...
  return (
    <div className="auth-page">
      <div className="auth-container">
        ...
        <p className="auth-subtitle">Регистрация аналитика</p>
        <p className="auth-subtitle" style={{ fontSize: '18px', marginTop: '-15px', marginBottom: '30px' }}>
          Начните получать заказы на проекты
        </p>
        ...
        <div className="auth-footer">
          <p>
            Уже есть аккаунт? <Link to="/fill">Войти</Link>
          </p>
        </div>
...
export default RegisterAnalystPage;